...
if args.mode == 'uncalib' or args.mode == 'mse':
    weights_path = 'weights/weights.%s.latest.hdf5'%(args.mode)
else:
    weights_path = 'weights/weights.%s.%0.3f.latest.hdf5'%(args.mode,args.reg)
...
def load_images(noise):
    basepath = "smalldataset/2-" + noise
    images = []
    for path in sorted(glob.glob(basepath + '/*.png')):
...
if args.mode == 'mse' or args.mode == 'uncalib':
    experiment_name = '%s'%(args.mode)
else:
    experiment_name = '%s.%0.3f'%(args.mode,args.reg)
